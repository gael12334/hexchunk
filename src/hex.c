...
//   // 2nd arg : range
//   long range;
//   err = a_arg2long(args->list[2], &range);
...
//
//   ps_t ps = p_decayed(args->list[1]);
//   err = p_init(&ha->hex.path, &ps);
//   check_he(err, { printf("Path is invalid; error code %i.\n", err); });
//
//   err = s_openfile(&ha->hex.stream, args->list[1], sm_binary_read);
//   check_he(err, {
...
//
//   long offset;
//   err = a_arg2long(args->list[1], &offset);
...
//
//   long size;
//   err = a_arg2long(args->list[1], &size);
//   check_he(err, { printf("Failed to parse offset; error code %i.\n", err); });
//
//   if (size < 0) {
...
//   check_he(err, {});
//
//   size_t length = strlen(args->list[1]);
//   int8_t* pattern = malloc(sizeof(int8_t) * length);
//   assert(pattern != NULL);
//   memcpy(pattern, args->list[1], length);
